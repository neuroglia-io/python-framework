...
class JavaScriptExpressionTranslator:
    ...
    def translate(self, expression: expr) -> str:
        ...
        if isinstance(expression, arg):
            return self._translate_arg(expression)
        elif isinstance(expression, Attribute):
            return self._translate_attribute(expression)
        elif isinstance(expression, BoolOp):
            return self._translate_bool_op(expression)
        elif isinstance(expression, Call):
            return self._translate_call(expression)
        elif isinstance(expression, Compare):
            return self._translate_compare(expression)
        elif isinstance(expression, Constant):
            return self._translate_constant(expression)
        elif isinstance(expression, Lambda):
            return self._translate_lambda(expression)
        elif isinstance(expression, Name):
            return self._translate_name(expression)
        elif isinstance(expression, Subscript):
            return self._translate_subscript(expression)
        elif isinstance(expression, UnaryOp):
            return self._translate_unary_op(expression)
        elif isinstance(expression, ast.List):
            return self._translate_list(expression)
        else:
            raise Exception(f"The specified expression type '{type(expression)}' is not supported in this context")
    ...
    def _translate_call(self, expression: Call) -> str:
        ...
        if function_name == "count":
            return f"{function_body}.length"
        elif function_name == "len":
            return f"{function_body}.length"
        elif function_name == "lower":
            return f"{function_body}.toLowerCase()"
        elif function_name == "startswith":
            return f"{function_body}.startsWith({function_args[0]})"
        elif function_name == "endswith":
            return f"{function_body}.endsWith({function_args[0]})"
        elif function_name == "index":
            return f"{function_body}.indexOf({', '.join(function_args)})"
        elif function_name == "upper":
            return f"{function_body}.toUpperCase()"
        elif function_name == "split":
            return f"{function_body}.split({', '.join(function_args)})"
        else:
            raise Exception(f"The specified function '{function_name}' is not supported in this context")
    ...
    def _translate_subscript(self, expression: Subscript) -> str:
        ...
        try:
            slice_index = int(slice)
        except:
            pass
        if slice_index is not None and slice_index < 0:
            return f"{source}.slice({slice})"
        else:
            return f"{source}[{slice}]"

    def _translate_unary_op(self, expression: UnaryOp) -> str:
        operand = self.translate(expression.operand)
        if isinstance(expression.op, Not):
            return f"!({operand})"
        elif isinstance(expression.op, USub):
            return f"-{operand}"
        else:
            raise Exception(f"The specified unary operator '{type(expression.op).__name__}' is not supported in this context")
    ...
    def _translate_boolop(self, operator: boolop) -> str:
        ...
        if result is None:
            raise Exception(f"The specified bool operator '{type(operator).__name__}' is not supported")
        return result
    ...
    def _translate_cmpop(self, operator: cmpop) -> str:
        ...
        result = switch.get(type(operator))
        if result is None:
            raise Exception(f"The specified comparison operator '{type(operator).__name__}' is not supported")
        return result
