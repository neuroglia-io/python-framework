...
class DomainEventHandlerBase(Generic[TWriteModel, TReadModel, TKey]):
    ...
    def __init__(self, mediator: Mediator, mapper: Mapper, write_models: Repository[TWriteModel, str], read_models: Repository[TReadModel, str], cloud_event_bus: CloudEventBus, cloud_event_publishing_options: CloudEventPublishingOptions):
    ...
    async def get_or_create_read_model_async(self, id: str):
        ...
        if read_model is None:
            ...
            if write_model is None:
                raise Exception(f"Failed to find a write model instance of type '{write_model_type}' with the specified key '{id}'")
    ...
    async def publish_cloud_event_async(self, e: DomainEvent):
        ...
        cloud_event = CloudEvent(id_, source, type_, spec_version, time, subject, data=integration_event)
