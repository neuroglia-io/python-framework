...
class CreateBankAccountTransferCommandHandler(CommandHandler[CreateBankAccountTransferCommand, OperationResult[BankTransactionDto]]):
    ...
    async def handle_async(self, command: CreateBankAccountTransferCommand) -> OperationResult[BankTransactionDto]:
        if command.from_bank_account_id == command.to_bank_account_id: return self.bad_request("Cannot perform a transfer from a bank account to itself")
        
        from_bank_account = await self.bank_accounts.get_async(command.from_bank_account_id)
        if from_bank_account is None: return self.not_found(BankAccount, command.from_bank_account_id)
        
        to_bank_account = await self.bank_accounts.get_async(command.to_bank_account_id)
        if to_bank_account is None: return self.not_found(BankAccount, command.to_bank_account_id)
        ...
        if not from_bank_account.try_add_transaction(transaction): return self.bad_request("Insufficient funds")
        if not to_bank_account.try_add_transaction(transaction): raise Exception("The beneficiary of a transaction of type 'TRANSFER' should never invalidate it for insufficient funds, thus this exception should never be raised")
